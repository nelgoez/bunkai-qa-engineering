...
This repository runs `solo-main`: single maintainer (Nahuel Gomez — QA), commit and push directly to `main`. This is a CHOSEN strategy (`strategy_source: chosen`), not the inherited default — do not re-offer Strategy Setup here unless the user asks to change the strategy.

### Host policy — no accepted divergences

This repo has **no accepted divergences**: `bun run git:policy verify` passes with **0 drift** — the host ruleset matches `git_strategy.policy` exactly (`direct_push_to_protected: allowed`, `require_pr_reviews: 0`, no `pull_request` rule on `main`). `meta.policy_source: verified` records that (distinct from `accepted`, which is used only when a formal `accepted_divergences` entry is declared).

Operational consequences:

- **`verify` exits 0** (0 drift) and runs automatically in the pre-push hook and in `bun run repo:check`. Unreachable host = warn + exit 0 (absence of data is not drift).
- **`bun run git:policy apply`** derives the host's ruleset as-is; always read the dry run before `--yes`.
