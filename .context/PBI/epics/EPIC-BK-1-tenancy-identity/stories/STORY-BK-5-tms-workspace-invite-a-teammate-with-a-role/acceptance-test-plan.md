...
- **INT-1: Should return invites array with correct shape** — ✅ PASSED. Fields: id, email, role, status, expires_at, created_at, accepted_at, revoked_at.
...
- **DB-1: DB records match API responses** — ✅ PASSED. 6 invites in DB match GET response. Revoked_at timestamps correct. invited_by_user_id consistent.
...
Invite can be created for an email that already belongs to an active workspace member. Spec requires 409 EMAIL_ALREADY_MEMBER.
...
Accepting an invite for member role when user is already owner DEmotes existing membership. The upsert `onConflict: workspace_id,user_id` always sets role to invite.role. Fix: only upsert if no existing row, or preserve higher existing role.
