...
> **Refresh deltas (2026-09-15)**: auth now includes email+password two-step (`/auth/signup`·`/auth/confirm`·`/auth/signin`) alongside magic-link + GitHub/Google OAuth (was OAuth+magic-link only); dashboard = `/home` (BK-254) + project `/metrics` (BK-46/BK-47); no `integrations`/`environments` tables (`import_jobs` / `project_environments`); API is fully implemented (84 handlers).
...
| -------- | ----------------------------------------------------------- | --- | ------------------- | --- | ------------------------------ | --------------- |
| FEAT-001 | Email + password two-step + OAuth sign-up (GitHub / Google) + magic-link | 1.1 | {{PROJECT_KEY}}-001 | yes | Community / Cloud / Enterprise | Elena, Sara     |
