...
> **Last update**: 2026-09-15 (post-implementation refresh; prior 2026-05-19)
> **Last verified against OpenAPI on**: 2026-09-15 (live `../upex-bunkai-tms` — 84 `/api/v1` handlers; prior contract-first 2026-05-19)
> **Mode**: UPDATE (post-implementation — the API is now live; was contract-first synthesis)
> **Refresh deltas (2026-09-15)**: auth = email+password two-step + magic-link + GitHub/Google OAuth; no `integrations`/`environments` tables (Jira import = `import_jobs`, env = `project_environments`); analytics live on workspace/project report endpoints, not an `/analytics` namespace; dashboard = `/home` (BK-254).
...
The API is operated by two distinct consumer classes. **End users in browsers** sign in via Supabase Auth (email+password two-step, magic-link, or GitHub / Google OAuth), carry a JWT in an HttpOnly cookie, and resolve workspace context implicitly from session state. **API operators** (the Bunkai CLI, AI agents, CI/CD pipelines — the Karim persona) issue **Personal Access Tokens** (`bk_pat_<random32>`, SHA-256-hashed at rest) and present them as `Authorization: Bearer ...` on every request. Token scopes (`read|write`) layer on top of the workspace role hierarchy (`viewer ⊂ member ⊂ admin ⊂ owner`), so a `member` user can mint a read-only token whose bearer cannot create ATCs even though the underlying user could.
