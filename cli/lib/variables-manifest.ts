...
export const VAR_MANIFEST: VarSpec[] = [
  // --- Environment selection ---
  {
    ...
    obtainHint: 'defaults to staging; reconfigure manually or via the /adapt-framework skill when you adapt the framework to your project-under-test.',
    note: 'Which environment to test against (local | staging | production). CI env INPUT, not a secret; required by validateTestEnv.ts. Installer writes the default; never prompts.',
  },
  ...
  },
  {
    ...
    required: { ifEnv: 'TEST_ENV=staging' },
    critical: false,
    obtainHint: 'test-user PAT for your project-under-test; set when adapting the framework to your project.',
    note: 'Staging test user Personal Access Token — alternative to email+password (bypasses broken /auth/login, BK-177). Project-dependent — set later.',
  },
  {
    ...
    required: false,
    critical: false,
    obtainHint: 'restricted-scope test-user PAT for your project-under-test; set when adapting the framework.',
    note: 'Staging restricted-scope PAT (e.g. atc:read only). Optional.',
  },
  {
    name: 'PRODUCTION_USER_EMAIL',
    destinations: ['local', 'github'],
    secret: false,
    required: { ifEnv: 'TEST_ENV=production' },
    critical: false,
    obtainHint: 'production test-user creds for your project-under-test; set when adapting the framework.',
    note: 'Production test user email. Required when TEST_ENV=production. Project-dependent — set later.',
  },
  {
    name: 'PRODUCTION_USER_PASSWORD',
    destinations: ['local', 'github'],
    secret: true,
    required: { ifEnv: 'TEST_ENV=production' },
    critical: false,
    obtainHint: 'production test-user creds for your project-under-test; set when adapting the framework.',
    note: 'Production test user password. Required when TEST_ENV=production. Project-dependent — set later.',
  },
  {
    name: 'PRODUCTION_USER_PAT',
    destinations: ['local', 'github'],
    secret: true,
    required: { ifEnv: 'TEST_ENV=production' },
    critical: false,
    obtainHint: 'production test-user PAT for your project-under-test; set when adapting the framework.',
    note: 'Production test user Personal Access Token — alternative to email+password. Project-dependent — set later.',
  },
  {
    name: 'PRODUCTION_USER_READONLY_PAT',
    destinations: ['local', 'github'],
    secret: true,
    required: false,
    critical: false,
    obtainHint: 'production restricted-scope test-user PAT for your project-under-test; set when adapting the framework.',
    note: 'Production restricted-scope PAT (e.g. atc:read only). Optional.',
  },
