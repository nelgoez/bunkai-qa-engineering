...
| Transition Ready For QA → In Testing | `jira workitem transition --key <KEY> --status <STATUS>` | `<STATUS>` = `{{jira.status.story.in_test}}`. If acli returns "no allowed transitions", verify ticket is assigned to yourself first — REST-assign via `PUT /rest/api/3/issue/{KEY}/assignee {"accountId":"..."}` then re-fetch transitions. Fallback: REST `GET /rest/api/3/issue/{KEY}/transitions` → `POST /rest/api/3/issue/{KEY}/transitions {"transition":{"id":"<TRANSITION_ID>"}}"` with explicit ID from `.agents/jira-workflows.json`. See `acli/references/gotchas.md` §9. |
