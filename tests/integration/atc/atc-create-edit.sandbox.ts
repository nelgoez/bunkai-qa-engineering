...
test.describe('BK-18: ATC Create/Edit REST API', { tag: ['@api', '@atc', '@critical'] }, () => {
  ...
  test.fixme('BK-150: POST /atcs rejects token lacking atc:write scope with 403', async ({ api }) => {
    // TODO: needs STAGING_USER_READONLY_PAT env var — a PAT from a user who
    // authenticates successfully but lacks atc:write scope.
    // Current STAGING_USER_PAT is full-scope, so 403 cannot be tested yet.
    const readonlyPat = process.env.STAGING_USER_READONLY_PAT;
    if (!readonlyPat) {
      test.skip(true, 'STAGING_USER_READONLY_PAT not configured');
    }
